...
// GET: list all brands or get one by ?id=...
export async function GET(request) {
  try {
    ...
    const list = await prisma.brands.findMany({ orderBy: { createdAt: 'desc' } })
...
export async function POST(request) {
  try {
    ...
    const created = await prisma.brands.create({ data: { name } })
...
export async function PUT(request) {
  try {
    ...
    const updated = await prisma.brands.update({ where: { id }, data: { ...(name ? { name } : {}) } })
