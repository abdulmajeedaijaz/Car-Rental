...
// GET: list all models or get one by ?id=...
export async function GET(request) {
  try {
    ...
    const list = await prisma.models.findMany({
      include: { brand: true },
...
export async function POST(request) {
  try {
    ...
    if (!brand_id) return NextResponse.json({ error: 'Missing `brand_id` in request body' }, { status: 400 })
...
// PUT: update an existing model (body must include `id`)
